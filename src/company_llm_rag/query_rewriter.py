...
- [중요] 이전 대화가 있으면 이번 입력은 후속 질문일 가능성이 높습니다. 후속 질문이 문법적으로
  완결된 문장이어도, 이전 턴에서 다루던 주제어(기능명·대상)를 rewritten과 keywords에 반드시
  포함해 그 자체로 이해되는 독립적 검색문으로 만드세요. 이전 주제와 무관한 새 질문일 때만 생략합니다.
  예) 직전 턴 "쿠폰 난수번호 대소문자 구분해야 하나?" → 후속 "구분 문제 해결을 위한 안내는?"
      → rewritten: "쿠폰 난수번호 대소문자 구분 문제 해결 안내", keywords: ["쿠폰", "난수번호", "대소문자"]
