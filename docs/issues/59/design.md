...
## Phase 1 — 공짜 그래프: Jira 구조 활용 ✅ 구현 완료 (2026-08-22)

구현 결과: `graph/` 모듈 (graph_store·jira_graph·query_router), 이슈 25,284건 →
노드 27,245·엣지 91,106. data_loader가 야간 적재 후 자동 재구축.
설계와 달라진 점: LLM이 목록·건수를 서술하지 않음 — 기준·총건수·목록은 서버가
확정 조립하고 LLM은 경향 요약 1~2문장만 담당 (LLM 건수 오류 실측 후 변경).
...
설계와 달라진 점: ①엔티티 사전은 DB(entities 테이블) + 관리자 설정 탭 편집 UI
(조회/저장/링크 재구축, 2026-08-22 추가 — SEED는 최초 시드로만 사용) ②링크 매칭은
...
**매뉴얼을 엔티티 사전의 백본**으로 사용한다. 매뉴얼은 담당자가 검수한
구조화 자산이라 LLM 추출 없이 사전을 만들 수 있다.

1. **엔티티 사전 구축 — 매뉴얼에서 기계 추출**:
   - 기능 ID 체계 (`TBL-C-01`, `STL-A-24`, `ST-ADM-` 등 접두 패턴)
   - 문서 제목·헤딩의 표준 용어 + 개요의 동의어 정리("테이블오더·QR주문·스마트오더")
   - 도메인 축 = 매뉴얼 문서 단위 (주문/정산/구독/회원/알림/POS/업체)
   → `entities` 테이블 (별칭 포함). 관리자 편집 가능, 질문 로그·위키 토픽은 보강 소스
2. **문서-엔티티 링크**: 로더에서 신규/변경 문서만 대상으로
   - 1차: 사전 기반 문자열/별칭 매칭 (LLM 불필요, 전 문서 커버)
   - 2차(선택): 매칭 애매한 문서만 경량 LLM 확인
   → `graph_edges(doc→entity, MENTIONS)` — 매뉴얼 문서 자신도 엔티티의 허브 노드
3. **서빙**:
   - 매뉴얼 문서가 검색에 걸리면 같은 엔티티에 링크된 Jira/Confluence 문서를
     검색 후보에 주입 (기존 extra_queries 패턴) → "정책 현황(매뉴얼) + 변경 이력(Jira)" 멀티홉
   - 엔티티 중심 질문("X 관련 정책 전부") → 엔티티 링크 문서 집합 주입
   - 멀티홉 확장은 엔티티-엔티티 동시출현 edge
