...
설계 문서(`docs/issues/37_question_group_session_redesign_2026-03-25.md`)의 테스트 케이스를 실제 서버에서 검증 필요.

| TC | 항목 | 상태 |
|---|---|:---:|
| TC-01 | 새 질문 시작 시 새 세션 생성 | 미검증 |
| TC-02 | 후속 질문은 같은 세션 유지 | 미검증 |
| TC-03 | 다른 새 질문은 다른 세션으로 분리 | 미검증 |
| TC-04 | 그룹 피드백 저장 | 미검증 |
| TC-05 | 그룹 피드백과 턴 피드백 분리 | 미검증 |
| TC-06 | 그룹 분석 수행 | 미검증 |
| TC-07 | 관리자 그룹 목록 조회 | 미검증 |
| TC-08 | 관리자 그룹 상세 조회 | 미검증 |
| TC-09 | 기존 데이터 마이그레이션 | 미검증 |
| TC-10 | Teams 문의와 그룹 세션 호환 | 미검증 |
