...
try:
    ...
    print(f"Error: Collection '{COLLECTION_NAME}' not found. Please run data_loader.py first. {e}", file=os.stderr)
    exit(1)
...
def retrieve_documents(query: str, n_results: int = 5) -> List[Dict]:
    ...
    try:
        ...
        print(f"Error during document retrieval: {e}", file=os.stderr)
        return []
...
--- Retrieved Documents ---")
            for i, doc in enumerate(results):
                ...
                print(f"  Content (chunk): {doc['content'][:200]}...") # Print first 200 chars
